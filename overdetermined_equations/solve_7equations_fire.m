function [best_solution, best_res] = solve_7equations_fire(x0, y0, alpha, phi1, phi2, phi3, D1, D2, K1, K2)
    % 参数设置
    initial_temp = 100000;    % 初始温度
    cooling_rate = 0.99;    % 冷却速率
    num_iterations = 100000;  % 每个温度下的迭代次数,注意控制迭代次数
    min_temp = 1e-6;        % 最小温度
    rng('shuffle'); %还是要定义一下
    % 定义变量的上下界
    bounds = [
    ...
    current_solution = [ 0 , 0 , 0 , 0 ,0 ,0,-6];
    %current_solution = [ 2.28425477 ,-1.71574523 , 0.70018338 , 1.04480039 ,-0.21639455 ,-0.17768772,-5.15080758];
    %current_solution(1) = 2;
    %current_solution(2) = -2;
    %current_solution(3) = pi/4;
    %current_solution(4) = 1;
    %current_solution(5) = 0;
    %current_solution(6) = 0;
    %current_solution(7) = -6;
    % 计算当前解的残差
